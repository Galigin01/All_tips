...
//Часть 2  Решение не совсем верное, но рабочее и строго соответсвует заданию. Тут мы добавили метод, который addMethod, который переписывает метод calculate и добавляет в него опцию вычисления. Но тут добавляется только последняя добавленная опция, а предыдущая не сохраняется (вообщем-то в задании не было разъяснено этого момента)  аргумент func фактически муляж, который либо совпадает со стрелочной функцией с аргументами a и b, либо нет  Ошибка: split через пробел надо   function Calculator() {
...
// - создать аккумулятор равный пустому обьекту - в аккумулятор вставлять элементы (ключи) равные елементам редуцируемого обьекта - вернуть аккумулятор  Помнить: результат reduce() это обобщенный результат работы всех функций над каждым элементом обьекта 
...
// 
//Здесь очень удобно коллекцией Set мы убираем дубли  и используем Array.from
...
//собрали массивы в строки (наборы букв) строки привели к lowerCase Все что получилось собрали в коллекцию Set (т.е отфильтровали все дубли)   let arr = ["nap", "teachers", "cheaters", "PAN", "ear", "era", "hectares"];   function aclean(ar) {
return Array.from(new Set(ar.map(el => {
    return (Array.from(el).sort()).join('').toLowerCase();
})));
};  aclean(arr);//   В ответе учебника сделано так же,  но еще создали коллекцию Map и воспользовались методом set(key, value) для установки ключа и значения, получилось что в ключ добавили единственное значение, которое не может повторяться т.е в нашем случае это отсортированные слова по буквам,  а в ключи попали первые слова,  которые были для сортировки   т.е получилась вот такая коллекция, из которой потом мы взяли ключи и вставили массив  {"anp" => "PAN"} {"aceehrst" => "hectares"} {"aer" => "era"}   let aclean = function(ar) {
let map = new Map;
 ar.forEach(el => {
...
//////////////////////////////////////////////////////////////////////////////////////////// 
// https://learn.javascript.ru/keys-values-entries    1 задание:  
 let salaries = {
  "John": 100,
  "Pete": 300,
  "Mary": 250
...
//https://learn.javascript.ru/destructuring-assignment#destrukturizatsiya-obekta   1 задание:  
let user = {
  name: "John",
  years: 30
};
...
meetup.self = meetup; // тут получается зацикливание, что не допустимо при серриализации  Ответ:
